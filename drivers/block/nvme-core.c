...
static inline void nvme_release_cmb(struct kref *kref);

static inline void nvme_free_sq_cmds(struct nvme_queue *nvmeq)
{
	if (nvmeq->cmb_mapped)
		kref_put(&nvmeq->dev->cmb_ref, nvme_release_cmb);
}
				...
				int entry_size)
{
	...
	unsigned q_size_aligned = roundup_pow_of_two(q_depth * entry_size);
	if (q_size_aligned * nr_io_queues > dev->cmb_size) {
		q_depth = rounddown_pow_of_two(dev->cmb_size / nr_io_queues /
						entry_size);
		q_depth = min_t(int, q_depth, dev->q_depth);
		q_size_aligned = roundup_pow_of_two(q_depth * entry_size);
	}

	if (q_size_aligned < dev->page_size)
		return -ENOMEM;
				...
				int qid, int depth)
{
	if (qid && NVME_CMB_SQS(dev->cmbsz) && dev->cmb) {
		unsigned offset = (qid - 1) *
					roundup_pow_of_two(SQ_SIZE(depth));
		nvmeq->cmb_mapped = true;
		nvmeq->sq_dma_addr = dev->cmb_dma_addr + offset;
		kref_get(&dev->cmb_ref);
...
static inline void nvme_release_cmb(struct kref *kref)
{
	struct nvme_dev *dev = container_of(kref, struct nvme_dev, cmb_ref);
	if (dev->cmb)
		iounmap(dev->cmb);
}
...
static int nvme_setup_io_queues(struct nvme_dev *dev)
{
	...
	if (NVME_CMB_SQS(dev->cmbsz) && dev->cmb) {
		...
		if (result > 0)
			...
			dev->cmbsz &= ~(NVME_CMB_SQS(dev->cmbsz));
	}
...
static int nvme_dev_start(struct nvme_dev *dev)
{
	...
	if (dev->cmb)
		kref_put(&dev->cmb_ref, nvme_release_cmb);
