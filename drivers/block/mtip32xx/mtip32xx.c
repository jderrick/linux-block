...
static struct mtip_cmd *mtip_cmd_from_tag(struct driver_data *dd,
					  unsigned int tag)
{
	return mtip_rq_from_tag(dd, tag)->special;
}
...
static struct blk_mq_reg mtip_mq_reg = {
	.ops		= &mtip_mq_ops,
	.nr_hw_queues	= 1,
	.queue_depth	= MTIP_MAX_COMMAND_SLOTS,
	.reserved_tags	= 1,
...
static void mtip_free_cmd(void *data, struct blk_mq_hw_ctx *hctx,
...
static int mtip_init_cmd(void *data, struct blk_mq_hw_ctx *hctx,
			 struct request *rq, unsigned int i)
{
	struct driver_data *dd = data;
	struct mtip_cmd *cmd = rq->special;
...
skip_create_disk:
	...
	rv = blk_mq_init_commands(dd->queue, mtip_init_cmd, dd);
	if (rv) {
		dev_err(&dd->pdev->dev,
			"Unable to initialize hw commands\n");
...
read_capacity_error:
init_hw_cmds_error:
	blk_mq_free_commands(dd->queue, mtip_free_cmd, dd);
	blk_cleanup_queue(dd->queue);
